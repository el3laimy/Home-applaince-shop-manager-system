...
/// Applies: N+1 fix, atomic stock/cost updates, and safe post-commit cache invalidation.
/// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var item in dto.Items)
            {
                ...
                if (!isDraft)
                {
                    // ── 3. WAC & Stock — atomic update, no EF state tracking ────
                    // WAC = ((Old Qty * Old Price) + (New Qty * New Price)) / (Old Qty + New Qty)
                    var quantity = item.Quantity;
                    var unitCost = item.UnitCost;
                    var totalQuantity = product.StockQuantity + quantity;
                    var newWac = totalQuantity <= 0
                        ? product.PurchasePrice
                        : ((product.StockQuantity * product.PurchasePrice) + (quantity * unitCost)) / totalQuantity;

                    await _dbContext.Set<Product>()
                        .Where(p => p.Id == product.Id)
                        .ExecuteUpdateAsync(setters => setters
                            .SetProperty(p => p.StockQuantity, p => p.StockQuantity + quantity)
                            .SetProperty(p => p.PurchasePrice, newWac),
                            cancellationToken);
                    var refreshedQty = await _dbContext.Set<Product>()
                        .Where(p => p.Id == product.Id)
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var item in invoice.Items)
            {
                ...
                var newWac = totalQuantity <= 0
                    ? product.PurchasePrice
                    : ((product.StockQuantity * product.PurchasePrice) + (quantity * unitPrice)) / totalQuantity;
