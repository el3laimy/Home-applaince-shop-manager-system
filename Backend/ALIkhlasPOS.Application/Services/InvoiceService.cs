...
public class InvoiceService : IInvoiceService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Atomic Sequence for Invoice Number
            var today = DateTime.UtcNow.ToString("yyyyMMdd");
            var result = await _dbContext.Database
                .SqlQueryRaw<long>("SELECT nextval('invoice_seq') AS \"Value\"")
                .FirstAsync(cancellationToken);
                
            string invoiceNo = $"INV-{today}-{result:D5}";
            ...
            foreach (var scan in groupedScans)
            {
                ...
                if (bundleComponents.Any())
                {
                    foreach (var bundleItem in bundleComponents)
                    {
                        if (subProducts.TryGetValue(bundleItem.SubProductId, out var subProduct))
                        {
                            ...
                            // 3. Concurrency fix: atomic stock decrement
                            int rowsAffected = await _dbContext.Set<Product>()
                                .Where(p => p.Id == subProduct.Id && p.StockQuantity >= totalSubQtyRequired)
                                .ExecuteUpdateAsync(setters => setters
                                    .SetProperty(p => p.StockQuantity, p => p.StockQuantity - totalSubQtyRequired),
                                    cancellationToken);
                ...
                else
                {
                    // 3. Concurrency fix: atomic stock decrement
                    int rowsAffected = await _dbContext.Set<Product>()
                        .Where(p => p.Id == product.Id && p.StockQuantity >= quantity)
                        .ExecuteUpdateAsync(setters => setters
