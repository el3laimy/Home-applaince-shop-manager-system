...
public class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Invoice>().Property(i => i.VatRate).HasColumnType("numeric(7,4)");
        ...
        // --- Shop Settings ---
        modelBuilder.Entity<ShopSettings>().Property(s => s.DefaultVatRate).HasColumnType("numeric(7,4)");
    ...
    public override async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            if (entry.State == EntityState.Added)
            {
                ...
                foreach (var prop in entry.Properties.Where(p => !IsSensitiveAuditProperty(p.Metadata.Name)))
                {
            ...
            else if (entry.State == EntityState.Modified)
            {
                ...
                foreach (var prop in entry.Properties.Where(p => p.IsModified && !IsSensitiveAuditProperty(p.Metadata.Name)))
                {
