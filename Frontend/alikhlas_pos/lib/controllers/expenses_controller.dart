...
class ExpensesController extends GetxController {
  ...
  Future<void> fetchExpenses() async {
    ...
    try {
      ...
      if (filterEndDate != null) {
        query +=
            '${query.isEmpty ? '?' : '&'}endDate=${filterEndDate!.toIso8601String()}';
      }
    ...
    try {
      ...
      if (localReceiptPath != null && localReceiptPath.isNotEmpty) {
        ...
        try {
          ...
          Get.snackbar(
            'تحذير',
            'فشل في رفع صورة الإيصال، سيتم تسجيل المصروف بدونه',
  ...
  Future<bool> createCategory(String name) async {
    try {
      final response = await ApiService.post('ExpenseCategories', {
        'name': name,
      });
      ...
      Get.snackbar(
        'خطأ',
        'فشل في إضافة التصنيف أو أنه موجود بالفعل',
        backgroundColor: Colors.red.withAlpha(200),
        colorText: Colors.white,
      );
