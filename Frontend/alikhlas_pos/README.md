...
- `schemaVersion` الحالي: 11.
- v5 أضاف `products.imagePath`، وv6 مستندات تسوية الجرد، وv7 الأرصدة الافتتاحية، وv8 مرتجع المشتريات، وv9 تصحيح الخزينة والمحفظة، وv10 أساس قيمة المخزون والتكلفة التاريخية الدقيقة، وv11 مستند عكس التصحيح المالي وقيده المرجعي.
...
- `test/v2/v2_use_cases_test.dart`: منطق v2، الدفتر، المخزون، الأقساط، المرتجعات، full owner day، backup/restore، وترقيات المخطط المدعومة.
...
- `test/v2/financial_correction_idempotency_test.dart`: تسجيل التصحيح وعكسه بمستند مستقل وقيد معاكس، ومنع التكرار والعكس الثاني، والاستعادة والرجوع الذري.
- `test/v2/product_csv_import_test.dart`: معاينة CSV والأخطاء الصفية والحفظ الذري ومنع التكرار والاستئناف بعد فقد الرد.
- `test/v2/diagnostic_logger_test.dart`: حجب البيانات الخاصة من سجل الأعطال، وتدوير الملف، وربط الفشل غير المتوقع ببصمة العملية.
...
ينشئ Workflow `Desktop Release Build` مُثبّت Windows وحزمة Debian، ثم يشغّل دورة التثبيت والترقية والإزالة وإعادة التثبيت مع التحقق من بقاء بيانات المستخدم. لا تعتمد حزمة تسليم إلا إذا كان التشغيل أخضر وcommit المعروض داخل شاشة المساعدة يطابق commit التشغيل. خطوات القبول على الأجهزة الفعلية موثقة في `../../FIELD_ACCEPTANCE_AR.md`.
